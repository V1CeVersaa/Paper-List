...
4. `TODO` placeholders remaining in the body
5. `status: drafting` but content appears substantially complete
6. Missing `## 5. Related Work & Future Work` section
7. User opens the note and comments on quality or asks about it
...
5. If the note becomes complete, update `status: complete` in frontmatter.
6. Synchronize overview state: run `../read/scripts/sync_overview.py ... --checked` when the note is now complete.
7. Append ops log entry (`event=refresh`).
