...
This repository is a topic-first paper list built around nine skills. Four are primary creation workflows; five are supporting or read-only skills:

**Primary workflows**
- `/read`: read one paper and turn it into a finished note
- `/intake`: queue one or more papers, position them, and reserve their place in the topic overview
- `/explore`: bootstrap or expand a topic from a deep-research list or a domain discussion
- `/discuss`: discuss and iteratively develop an idea without forcing publication too early

**Supporting skills**
- `/maintain`: synchronize indexes, lint the repo, and run maintenance scripts
- `/refresh`: revise an existing note without treating it as a fresh read
- `/synthesize`: build comparison, survey, conference, or landscape-style outputs from existing notes

**Read-only skills**
- `/search`: find and rank existing notes and topics by relevance; never writes files
- `/review`: audit note quality and produce a diagnostic report; never writes files

The repository is organized around four storage layers with different publication and tracking semantics:

- `content/`: organized markdown artifacts that are meant to be browsed in Quartz
- `raw/`: immutable source layer such as PDFs and normalized source metadata; local only by default
- `inbox/`: structured queue items waiting to be read or promoted; local only by default
- `ops/`: local operational logs, registries, and reports; local only by default

Visibility and Git tracking are separate concerns:

- `public`: content is eligible for public publishing and belongs in tracked public paths such as `content/topics/` and `content/syntheses/`
- `local`: content is meant for local Quartz preview only and belongs in gitignored paths such as `content/local/`
- `draft`: content is unfinished working material and belongs in gitignored paths such as `content/drafts/`

Do not rely on frontmatter alone to keep private work off GitHub. Private work must stay in gitignored paths. Frontmatter only controls Quartz publication behavior.

When the user request does not explicitly name a workflow, choose the nearest match:

1. New paper + write note -> `/read`
2. Batch add papers + postpone reading -> `/intake`
3. New area + landscape building -> `/explore`
4. Open-ended research idea discussion -> `/discuss`

**Ambiguous cases — use these to break ties:**

- User provides a PDF/URL but does not say what to do → `/read` (most common intent)
- User says "加进去" / "以后再看" / "先存着" / "save for later" → `/intake`, regardless of whether a PDF is present
- User says "系统学一下 X" / "开新 topic" / "梳理这个领域" → `/explore`
- User is discussing a concept or idea without mentioning a specific deliverable → `/discuss`
- Request involves comparing multiple papers, writing a survey, recording conference observations, or updating a landscape → `/synthesize`
- User asks "有没有关于 X 的笔记" or wants to find existing content → `/search`
- User opens an existing note and asks to improve, clean, or update it → `/refresh`, not `/read`

## §1. Skill Reference

The primary repo-local skills live under `.codex/skills/`:

- `read`: single-paper read -> note -> promote
- `intake`: queue papers and reserve overview positions
- `explore`: bootstrap or expand a topic from a list
- `discuss`: create and evolve local-only idea files
- `maintain`: run synchronization, index rebuilds, and linting
- `refresh`: revise an existing note without treating it as a fresh read
- `synthesize`: build comparison or landscape-style outputs from existing notes
- `search`: find and rank existing notes/topics/syntheses by relevance (read-only, never writes files)
- `review`: quality-check existing notes and output a diagnostic report (read-only, optional)

Each skill owns its own workflow details, templates, and script entrypoints. This file is the dispatcher and policy layer.

### Skill Composition

Use these hand-offs to keep the workflows composable instead of re-implementing them ad hoc:

- `/read`: check whether a `status: complete` note already exists before starting — if it does, hand off to `/refresh`; after the full read, run a related-note pass against the target topic and nearby completed notes, verify whether the target topic still matches the paper's primary contribution before promotion, and finish by running `promote.py` then `sync_overview.py` in that order
- `/intake`: stop and hand off to `/explore` when the topic does not exist or the topic boundary is still unclear
- `/explore`: scaffold the topic first, then route the seed paper list through `/intake` instead of creating queue items manually — do not add overview entries by hand
- `/discuss`: keep the conversation free-form at first, but route reading gaps to `/intake` immediately when they appear; route mature comparative outputs to `/synthesize`
- `/refresh`: reuse `/read` maintenance steps (`sync_overview.py --checked`) when the refreshed note becomes complete
- `/maintain`: run after larger batches of `/read`, `/intake`, `/explore`, `/refresh`, or `/synthesize`
- `/search`: read-only; no hand-offs needed — recommend next skills to the user based on results but do not execute them automatically
- `/synthesize`: always read existing notes first; do not re-derive from raw PDFs unless no note exists

## §2. Script Inventory

The current script entrypoints are:

- `.codex/skills/intake/scripts/intake.py`
- `.codex/skills/read/scripts/promote.py`
- `.codex/skills/read/scripts/sync_overview.py`
- `.codex/skills/explore/scripts/create_topic.py`
- `.codex/skills/maintain/scripts/rebuild_index.py`
- `.codex/skills/maintain/scripts/lint_repo.py`
- `.codex/skills/maintain/scripts/note_length.py`
- `.codex/skills/maintain/scripts/query_context.py`
- `.codex/skills/maintain/scripts/migrate_frontmatter.py`

Shared support code:

- `.codex/skills/maintain/scripts/repo_ops.py`

Full invocation examples, trigger conditions, and flag reference for each script: `.codex/skills/_shared/script-registry.md`

## §3. Paper Note Writing Guide
...
> This writing guide is the authoritative specification. A mirror copy is maintained at `.codex/skills/read/references/writing-guide.md` for convenience when loading only the `/read` skill context.

## §4. Content Types & Frontmatter

### Content Types

- `content/topics/<topic>/index.md`: topic landing page
- `content/topics/<topic>/overview.md`: paper queue plus reading progress for the topic
- `content/topics/<topic>/landscape.md`: topic map, evolution, reading roadmap, and open questions
- `content/topics/<topic>/<paper>.md`: polished paper note
- `content/syntheses/<name>.md`: reusable comparisons, reviews, and cross-topic summaries
- `content/conferences/index.md`: landing page for conference-level tracking
- `content/conferences/<venue-year>.md`: records of emerging directions, methods, workshop takeaways, and oral-session observations from a specific conference
- `content/conferences/<venue-year>/index.md`: conference paper list when a venue needs many grouped entries
- `content/conferences/<venue-year>/<paper>.md`: conference-local paper note used before final topic归档
- `content/local/ideas/<name>.md`: local-only idea pages rendered in local Quartz builds
- `content/drafts/<...>.md`: unfinished drafts that should not be published or tracked
- `inbox/<slug>.md`: structured queue item for a paper or source waiting to be read

### Frontmatter Conventions

Use these fields when the page type supports them:

- `title`: short title or identifier
- `headline`: full paper title when applicable
- `description`: short summary for Quartz descriptions and search
- `status`: `queued | located | drafting | complete | stale`
- `visibility`: `draft | local | public`
- `topic`: canonical topic slug such as `reinforcement_learning`
- `venue`: venue string such as `ICLR 2016`
...
Path and visibility must agree:

- files under tracked public paths should generally use `visibility: public`
- files under `content/local/` should generally use `visibility: local`
- files under `content/drafts/` should generally use `visibility: draft`

### Publication Policy

- `public` content may be committed and deployed
- `local` and `draft` content must stay in gitignored local paths unless the user explicitly changes the policy
- if an item is ready to move from local or draft into public content, use `.codex/skills/read/scripts/promote.py`

### Source URL Policy

- `paper_url` is the canonical human-facing source link stored in frontmatter and inbox items
- `source_pdf` is the archived local file path under `raw/papers/`
- when archiving from a URL, normalize common paper pages into direct PDF downloads first:
  - `https://arxiv.org/abs/<id>` -> `https://arxiv.org/pdf/<id>.pdf`
  - `https://openreview.net/forum?id=<id>` -> `https://openreview.net/pdf?id=<id>`
- archive only PDF sources into `raw/papers/`; do not archive arXiv `e-print` URLs, TeX archives, HTML pages, or other source bundles as `source_pdf`
- do not guess unsupported publisher-specific PDF endpoints; store the original `paper_url` and leave `source_pdf` empty unless the agent has a verified downloadable PDF URL

## §5. Maintenance Protocols

Run the minimum maintenance for the workflow you executed:

- after `/read` for topic paper notes:
  - run the related-note pass before finalizing prose; inspect the topic page / overview and usually read one or two completed neighboring notes, then update `related` frontmatter and the Contributions positioning paragraph
  - run the post-read placement check; if the target topic changed, promote and sync against the corrected topic
  - `python3 .codex/skills/maintain/scripts/note_length.py --threshold 3500 <note-path>`
  - `python3 .codex/skills/read/scripts/promote.py ...`
  - `python3 .codex/skills/read/scripts/sync_overview.py ...`
  - append to `ops/log.md`
- after `/read` for conference-local paper notes:
  - run a lightweight related-note pass against the likely final topic when known, and record the likely final-topic relationship in the note's Placement Check or Contributions positioning paragraph
  - run the post-read placement check across both the conference subcategory and the likely final topic
  - `python3 .codex/skills/maintain/scripts/note_length.py --threshold 3500 <note-path>`
  - update the conference checklist with `[x]` and `[Note](./<note>.md)`
  - append to `ops/log.md` with `event=read`
- after `/intake`:
  - `python3 .codex/skills/intake/scripts/intake.py ...`
  - `python3 .codex/skills/maintain/scripts/rebuild_index.py`
  - append to `ops/log.md`
- after `/explore`:
  - batch `/intake`
  - `python3 .codex/skills/maintain/scripts/rebuild_index.py`
  - update topic ordering if the user wants explicit explorer ordering
  - append to `ops/log.md`
- after `/discuss`:
  - update the idea page
  - add connections to related notes when useful
  - append to `ops/log.md`
- after `/synthesize`:
  - update the synthesis, landscape, or conference-level observation page
  - link back to the source notes that the synthesis depends on
  - append to `ops/log.md` with `event=synthesize`

Run repository-wide checks after larger edits:
...
- public preview/build: `npm run site:dev` or `npm run site:build`
- local private preview/build: `npm run site:dev:private` or `npm run site:build:private`

The private build mode sets `QUARTZ_PRIVATE=1`, which allows `visibility: local` and `visibility: draft` pages to render locally. These files still must remain in gitignored paths if they should never reach GitHub.
