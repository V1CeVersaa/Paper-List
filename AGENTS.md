...
- `/read`: check whether a `status: complete` note already exists before starting — if it does, hand off to `/refresh`; finish by running `promote.py` then `sync_overview.py` in that order
