...
>
> 本文提出了一种无监督的、状态依赖的示范者专业度评估框架 ILEED，在无需信号和人工标注的情况下，仅收到异构质量数据以及其来源序号，将每一个示范者在每一个状态下的行为解释为 「围绕一个潜在最优策略的噪声化版本」，并通过最大似然学习联合学习技能嵌入和人员能力，从而从多示范者数据中恢复出最优策略。理论上证明了在状态嵌入存在线性纠缠的条件下，该方法具有可识别性，能唯一恢复出 Ground Truth 策略。
>
> 但是 ILEED 支持的模型较为有限，将次优策略建模成最优策略和均匀随机策略的插值形式，无法处理更复杂的多专家示范数据，比如存在系统性偏差或者多意图、异质观测的情况。此外 ILEED 对于状态嵌入的学习也存在较强假设，理论保证依赖于是否可以学到 「正确的状态嵌入」
